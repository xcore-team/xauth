...
def rbac_router(db: Any, cache: Any = None) -> APIRouter:
    ...
    async def create_role(
        body: RoleCreate,
        _: AuthPayload = Depends(require_permission("role:create")),
    ) -> Any:
    ...
    async def list_roles(
        ...
        _: AuthPayload = Depends(require_permission("role:list")),
    ) -> Any:
        async with db.session() as session:
    ...
    async def assign_role_to_member(
        ...
        _: AuthPayload = Depends(require_permission("role:update")),
    ) -> Any:
        async with db.session() as session:
            try:
    ...
    async def list_permissions(
        _: AuthPayload = Depends(require_permission("permission:list")),
    ) -> Any:
        async with db.session() as session:
